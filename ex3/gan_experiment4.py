...
from ex3.GeneratorConv import GeneratorConv
from ex3.GeneratorConv2 import GeneratorConv2

from ex3.DiscriminatorConv import DiscriminatorConv
...
from biotorch.module.biomodule import BioModule as BioModule
...
def run_experiment(dataloader, plot, num_epochs, dims, dims_total):
    ...
    generator = Generator(in_features=100, out_features=28*28, mode=mode, sig=False)
    discriminator = Discriminator(in_features=28*28, out_features=2, mode=mode, sig=False)
    ...
    #optimizerD = optim.SGD(gan.module.discriminator.parameters(), lr=lr_d, momentum=0.99)
    #optimizerG = optim.SGD(gan.module.generator.parameters(), lr=lr_g, momentum=0.99)
    criterion = nn.BCELoss()


    # True and False Labels.  128 is the batch size
    true_label = torch.ones(128, 2).to(device)
    true_label_g = torch.ones(128, 2).to(device)
    false_label = torch.zeros(128, 2).to(device)

    # Create folder to hold result
    result_folder = 'gan-result-partially' if USE_SIGMOID else 'gan-result-partially'
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx, (data, targets) in enumerate(dataloader):
            ...
            if ((epoch % 2 != 0 and batch_idx % 2 != 0) or (epoch % 2 == 0 and batch_idx % 2 == 0)) or free_mode:
            ...
            gan.module.generator.train()
            gan.module.discriminator.eval()
            gan.module.generator.zero_grad()
            g_fake_predict = gan(g_fake_noise, true_label, criterion)
            generator_loss = criterion(g_fake_predict, true_label_g)
            generator_batch_loss += generator_loss.item()
            generator_loss.backward()
            optimizerG.step()
            gan.module.discriminator.train()
            ...
            if (batch_idx + 1) % 100 == 0:
                ...
                if algorithm != "backpropagation" and algorithm != "dfa":
                    ...
                    print(alignment)"""
                    """alignment = gan.module.generator.hidden[0].compute_alignment()
                    print(alignment)
                    alignment = gan.module.discriminator.hidden[0].compute_alignment()
                    print(alignment)
                    alignment = gan.module.generator.hidden[0].compute_weight_ratio()
                    print(alignment)
                    alignment = gan.module.discriminator.hidden[0].compute_weight_ratio()
                    print(alignment)"""
        ...
        #torch.save(gan.module.generator, "generator_model_sigmoid.pt" if USE_SIGMOID else "generator_model.pt")
        #torch.save(gan.module.discriminator, "discriminator_model_sigmoid.pt" if USE_SIGMOID else "discriminator_model.pt")
